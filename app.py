...
client = MongoClient("mongodb+srv://admin:Z83J4sJNBQSKXjkC@cluster.fmcl4am.mongodb.net/naomi?retryWrites=true&w=majority")
db = client.naomi

template_folder = "templates/"
elements_folder = "elements/"
app = Flask(__name__, template_folder=template_folder)
app.config.update(SECRET_KEY="Ci serve energia dalle stelle")

@app.route('/', methods=['GET'])
def home():
    if request.method == 'GET':
        if request.args.get('cookies') is not None:
            session['cookies'] = True
    return render_template("index.html")

@app.route('/styler')
def styler():
    return send_file(template_folder+"style.css")
...
def form():
    ...
    if request.method == 'POST':
        ...
        infos = ['site_url', 'description', 'budget', 'price', 'sales', 'contact']
        for info in infos:
            if info in request.form:
                submission[info] = request.form[info]
            else:
                submission[info] = ""
        submission['tryied'] = request.form.getlist('tryied')
        db.forms.insert_one(submission)
        return render_template("registered.html")
    return render_template("form.html")

@app.route('/element/<element>')
def elements(element):
    return send_file(elements_folder+element)

if __name__ == '__main__':
   app.run(host="0.0.0.0", port=443, ssl_context=('cert.pem', 'privkey.pem'))
rn redirect(url_for('signin'))
            return False, False
